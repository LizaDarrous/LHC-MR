run_optim = function(par, start_ind, end_ind){
  ...
  test = optim(theta, LHC_SEM, 
               bX=bX[-(start_ind:end_ind)],bY=bY[-(start_ind:end_ind)],M=M, nX=nX, nY=nY, ld=ld[-(start_ind:end_ind)], wd=wd[-(start_ind:end_ind)], 
               method = "L-BFGS-B",
               lower = args.df[1,], 
               upper = args.df[2,],
  ...
  test.res=c(test$value,test$par, start_ind, end_ind)
  names(test.res)=c("mLL","iX", "iY", "pX","pU","pY","h2X","h2Y","tX","tY","a","b","rho","start_ind", "end_ind")
  return(test.res)
}
