run_optim = function(par){
  ...
  LHC_SEM <- function(theta,bX,bY,nX,nY,M,ld,wd){
    ...
    T1 = (h2X / (M*pX)) * matrix(c(1, a, a^2), ncol = 3, nrow = 1) * (1-(a*b))^2
    T2 = (1 / (M*pU)) * matrix(c((tX + (b*tY))^2, (tX + (b*tY)) * (tY + (a*tX)), (tY + (a*tX))^2), ncol = 3, nrow = 1) * (1-(a*b))^2
    T3 = (h2Y / (M*pY)) * matrix(c(b^2, b, 1), ncol = 3, nrow = 1) * (1-(a*b))^2
  ...
  names(test.res)=c("mLL","iX", "iY", "pX","pU","pY","h2X","h2Y","tX","tY","a","b","rho")
  return(test.res)
}